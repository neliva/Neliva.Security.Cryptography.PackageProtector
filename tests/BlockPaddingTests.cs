...
namespace Neliva.Security.Cryptography.Tests
{
    ...
    public class BlockPaddingTests
    {
        ...
        public void PKCS7PKCS7PaddingLengthBadArgumentFail()
        {
            ...
            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(4, null));
            Assert.Equal("data", ex.ParamName);
            Assert.Equal("Span is empty. (Parameter 'data')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(4, default));
            Assert.Equal("data", ex.ParamName);
            Assert.Equal("Span is empty. (Parameter 'data')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(4, Span<byte>.Empty));
            Assert.Equal("data", ex.ParamName);
            Assert.Equal("Span is empty. (Parameter 'data')", ex.Message);


            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(-1, new byte[16]));
            Assert.Equal("blockSize", ex.ParamName);
            Assert.Equal("Value must be between 1 and 255. (Parameter 'blockSize')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(0, new byte[16]));
            Assert.Equal("blockSize", ex.ParamName);
            Assert.Equal("Value must be between 1 and 255. (Parameter 'blockSize')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(byte.MaxValue + 1, new byte[16]));
            Assert.Equal("blockSize", ex.ParamName);
            Assert.Equal("Value must be between 1 and 255. (Parameter 'blockSize')", ex.Message);


            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(15, new byte[16]));
            Assert.Equal("data", ex.ParamName);
            Assert.Equal("Span length is not a multiple of block size. (Parameter 'data')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(17, new byte[32]));
            Assert.Equal("data", ex.ParamName);
            Assert.Equal("Span length is not a multiple of block size. (Parameter 'data')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Package.GetPKCS7PaddingLength(16, new byte[15]));
            Assert.Equal("data", ex.ParamName);
            Assert.Equal("Span length is not a multiple of block size. (Parameter 'data')", ex.Message);

            // Inner boundaries of the valid blockSize range must not throw.
            Assert.Equal(1, Package.GetPKCS7PaddingLength(1, new byte[] { 1 }));
            Assert.Equal(byte.MaxValue, Package.GetPKCS7PaddingLength(byte.MaxValue, CreateBuffer(byte.MaxValue, byte.MaxValue)));
        }
        ...
        public void PKCS7PaddingFullRangeZeroPadLengthFail()
        {
            for (int blockSize = 1; blockSize <= byte.MaxValue; blockSize++)
            {
                for (int blocks = 1; blocks <= 3; blocks++)
                {
                    ...
                    Assert.Equal(-1, Package.GetPKCS7PaddingLength(blockSize, b));
                }
            }
        }
            ...
            for (int blockSize = 1; blockSize <= byte.MaxValue; blockSize++)
            {
                for (int padLength = 1; padLength <= blockSize; padLength++)
                {
                    for (int blocks = 1; blocks <= 3; blocks++)
                    {
                        ...
                        Assert.Equal(padLength, Package.GetPKCS7PaddingLength(blockSize, b));
                    }
                }
            }
        }
        ...
        public void PKCS7PaddingFullRangeOversizedPadLengthFail()
        {
            for (int blockSize = 1; blockSize < byte.MaxValue; blockSize++)
            {
                for (int padLength = blockSize + 1; padLength <= byte.MaxValue; padLength++)
                {
                    ...
                    Assert.Equal(-1, Package.GetPKCS7PaddingLength(blockSize, single));

                    int blocks = (padLength + blockSize - 1) / blockSize;
                    byte[] multi = CreateBuffer(blockSize * blocks, (byte)padLength);

                    Assert.Equal(-1, Package.GetPKCS7PaddingLength(blockSize, multi));
                }
            }
        }
