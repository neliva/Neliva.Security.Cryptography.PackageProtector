...
namespace Neliva.Security.Cryptography.Tests
{
    ...
    public class BlockPaddingTests
    {
        ...
        public void PKCS7PKCS7PaddingLengthBadArgumentFail()
        {
            ...
            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Internals.GetPKCS7PaddingLength(-1, new byte[16]));
            Assert.Equal("blockSize", ex.ParamName);
            Assert.Equal("Value must be between 1 and 255. (Parameter 'blockSize')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Internals.GetPKCS7PaddingLength(0, new byte[16]));
            Assert.Equal("blockSize", ex.ParamName);
            Assert.Equal("Value must be between 1 and 255. (Parameter 'blockSize')", ex.Message);

            ex = Assert.Throws<ArgumentOutOfRangeException>(() => Internals.GetPKCS7PaddingLength(byte.MaxValue + 1, new byte[16]));
            Assert.Equal("blockSize", ex.ParamName);
            Assert.Equal("Value must be between 1 and 255. (Parameter 'blockSize')", ex.Message);
            ...
            Assert.Equal(1, Internals.GetPKCS7PaddingLength(1, new byte[] { 1 }));
            Assert.Equal(byte.MaxValue, Internals.GetPKCS7PaddingLength(byte.MaxValue, CreateBuffer(byte.MaxValue, byte.MaxValue)));
        }
        ...
        public void PKCS7PaddingFullRangeZeroPadLengthFail()
        {
            for (int blockSize = 1; blockSize <= byte.MaxValue; blockSize++)
            {
                for (int blocks = 1; blocks <= 3; blocks++)
                {
                    ...
                    Assert.Equal(-1, Internals.GetPKCS7PaddingLength(blockSize, b));
                }
            }
        }
            ...
            for (int blockSize = 1; blockSize <= byte.MaxValue; blockSize++)
            {
                for (int padLength = 1; padLength <= blockSize; padLength++)
                {
                    for (int blocks = 1; blocks <= 3; blocks++)
                    {
                        ...
                        Assert.Equal(padLength, Internals.GetPKCS7PaddingLength(blockSize, b));
                    }
                }
            }
        }
        ...
        public void PKCS7PaddingFullRangeOversizedPadLengthFail()
        {
            for (int blockSize = 1; blockSize < byte.MaxValue; blockSize++)
            {
                for (int padLength = blockSize + 1; padLength <= byte.MaxValue; padLength++)
                {
                    ...
                    Assert.Equal(-1, Internals.GetPKCS7PaddingLength(blockSize, single));

                    int blocks = (padLength + blockSize - 1) / blockSize;
                    byte[] multi = CreateBuffer(blockSize * blocks, (byte)padLength);

                    Assert.Equal(-1, Internals.GetPKCS7PaddingLength(blockSize, multi));
                }
            }
        }
