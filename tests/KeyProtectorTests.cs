...
namespace Neliva.Security.Cryptography.Tests
{
    ...
    public class KeyProtectorTests
    {
        ...
        // are fully reproducible during testing. When constructed without a
        // delegate, it defers to the default (cryptographically strong) RNG.
        private sealed class TestKeyProtector : KeyProtector
        {
            private readonly RngFillAction _fillRandom;

            public TestKeyProtector()
            {
            }
        ...
        public void ProtectBadContentSizeFails(int contentLength)
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => protector.Protect(content, package, password, iterations));
        ...
        public void UnprotectBadPackageSizeFails(int packageSize)
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => protector.Unprotect(package, content, password));
        ...
        public void UnprotectBadContentSpaceFails()
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => protector.Unprotect(package, context, password));
        ...
        public unsafe void ProtectBadAssociatedDataTooLargeFails(int associatedDataLength)
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => protector.Protect(content, package, password, 1, ad));
        ...
        public unsafe void UnprotectBadAssociatedDataTooLargeFails(int associatedDataLength)
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => protector.Unprotect(package, content, password, ad));
        ...
        public void RoundTripAcrossSeparateInstancesPass()
        {
            ...
            var pA = new TestKeyProtector();
            var pB = new TestKeyProtector();
